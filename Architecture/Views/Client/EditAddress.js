...
export default class EditAddress extends Component
 {
  ...
  componentWillMount = async () => {
    ...
    const { addType } = this.props.route.params;
    console.log('Details : ', passDetails);
    console.log('Type : ', addType);
    addressType = addType;
    addressArray = passDetails;
    console.log('Address Array : ', addressArray);
    console.log('Address Type : ', addressType);

    this.setState({title: addressArray.title});
    this.setState({address: addressArray.address});
    this.setState({city: addressArray.city});
    this.setState({state: addressArray.state});
    this.setState({zipCode: addressArray.zipcode});
...
validateZipCode = (text) =>{
  ...
  if (reg.test(text) === false) {
    ...
    editAddressDetails(){
      if (userType == "4" ){
      AddressViewModel.onUpdateClientAddressAPI(this.state.title,this.state.address,this.state.city,this.state.state,this.state.zipCode,addressType).then(
        (response,error) => {
        //get callback here
      ...
      AddressViewModel.onUpdateMechanicAddressAPI(this.state.title,this.state.address,this.state.city,this.state.state,this.state.zipCode,addressType).then(
        (response,error) => {
        //get callback here
    ...
    render() {
      ...
      return (
        ...
        <Text style={styles.titleText}>Edit Addresses</Text>
...
<View style={styles.inputContainer2}>
   ...
   />
              ...
              <TouchableOpacity style={[styles.buttonContainer, styles.connectButton]} 
                onPress={() => this.onClickEditListener()} >
                <Text style={styles.connectText}>Update Address</Text>
              </TouchableOpacity>
