...
export default class VehiclesList extends Component
 {
  ...
  state=
  {
    ...
    onAddingVehicleDetails = async () => {
      VehicleViewModel.onAddVehicleAPI(this.state.make, this.state.model, this.state.year).then(
        ...
        if (response.RESPONSE_CODE == 200){
          ...
          this.setState({ make : "" });
          this.setState({ year : "" });
          alert('Successfully added new vehicle details');
        }else{
          alert('Something went wrong, please try again');
        }
     });
    ...
    editActionOnRow(item) {
      console.log('Selected Item :',item);
      this.setState({vehicleSelected : item});
      this.props.navigation.navigate('EditProfileVehicle',{passProfileVehicle : item});
    }
    ...
    render() {
     ...
     renderItem={({ item}) => 
                {return(   
                  <View style={{marginTop: 15,marginBottom:15,marginLeft: 5,marginRight: 5,flexDirection:'column',justifyContent:'space-between',borderWidth:1,borderColor:'white',borderRadius:8}}>
                  <View style={{flexDirection:'row',marginTop:10,justifyContent:'space-evenly',textAlign:'center'}}>
                  ...
                  <View style={{flexDirection:'row',marginBottom:10,justifyContent:'space-evenly',marginBottom:10}}>
                  <TouchableOpacity onPress={ () => this.editActionOnRow(item)}>
                  <View style={{flexDirection:'row',justifyContent:'space-evenly',marginBottom:10}}>
                  <Image
                    style={{alignSelf:'flex-end',justifyContent:'center', marginTop:25, width:45, height : 45, borderRadius: 15}}
       ...
       textAlign:'center',}}> Edit </Text>
                  ...
                  <View style={{flexDirection:'row',justifyContent:'space-evenly',marginBottom:10}}>
                  <Image
                    style={{alignSelf:'flex-end', marginTop:25, width:45, height : 45, borderRadius: 15}}
